...
export default function RegisterPage() {
  ...
  const handleSendCode = async () => {
    if (!email.trim()) {
      toast.error("验证码发送失败", { description: "请先输入邮箱" });
      return;
    }
    setCodeLoading(true);
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      toast.error("注册失败", { description: t("auth.error.passwordMismatch") });
      return;
    }
    if (password.length < 6) {
      toast.error("注册失败", { description: t("auth.error.passwordMin") });
      return;
    }
        ...
        <section className="flex min-h-[calc(100vh-1.5rem)] items-center justify-center px-6 py-10 md:min-h-[calc(100vh-2.5rem)] md:px-10">
          <div className="w-full max-w-[430px] rounded-[36px] bg-white p-7 shadow-none md:p-8 lg:shadow-[0_28px_90px_rgba(15,23,42,0.08)]">
            <div className="pb-6 pt-2 text-center">
              ...
              <h1 className="text-[28px] font-semibold leading-tight tracking-[-0.04em] text-[#111827]">{t("auth.register.title")}</h1>
              <p className="mt-2 text-[14px] leading-5 text-[#6b7280]">{t("auth.modal.registerSubtitle")}</p>
            </div>

            <div className="mb-5 flex rounded-[22px] bg-[#f1f2f4] p-1.5 text-[15px] font-medium text-[#6b7280]">
              <Link href="/login" className="flex-1 rounded-[18px] px-4 py-2.5 text-center transition-all hover:text-[#111827]">
                {t("auth.login")}
              </Link>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t("auth.name.placeholder")} className={inputClassName} />
              </div>

              <div>
                <label className={labelClassName}>{t("auth.email")}</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required className={inputClassName} />
              </div>

              <div>
                <label className={labelClassName}>邮箱验证码</label>
                <div className="flex gap-2.5">
                  <input type="text" value={verificationCode} onChange={(e) => setVerificationCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="6 位验证码" required inputMode="numeric" className={`${inputClassName} min-w-0 flex-1`} />
                  <button type="button" onClick={handleSendCode} disabled={codeLoading || codeCountdown > 0} className="shrink-0 rounded-[18px] bg-[#111827] px-4 py-3.5 text-[14px] font-medium text-white transition-colors hover:bg-[#374151] disabled:cursor-not-allowed disabled:bg-[#d1d5db] disabled:text-[#6b7280]">{codeLoading ? "发送中" : codeCountdown > 0 ? `${codeCountdown}s` : codeSent ? "重发" : "发送"}</button>
                </div>
              </div>

              <div>
                <label className={labelClassName}>{t("auth.password")}</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.password.minPlaceholder")} required minLength={6} className={inputClassName} />
              </div>

              <div>
                <label className={labelClassName}>{t("auth.confirmPassword")}</label>
                <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t("auth.confirmPassword.placeholder")} required className={inputClassName} />
              </div>

              <button type="submit" disabled={loading} className="mt-2 flex w-full items-center justify-center gap-2 rounded-full bg-[#111827] px-5 py-3.5 text-[15px] font-semibold text-white transition-colors hover:bg-[#374151] disabled:cursor-not-allowed disabled:bg-[#d1d5db] disabled:text-[#6b7280]">
            ...
            <p className="mt-5 text-center text-sm text-[#9ca3af]">
              {t("auth.hasAccount")} <Link href="/login" className="font-medium text-[#111827] underline-offset-4 hover:underline">{t("auth.login")}</Link>
            </p>
          </div>
        </section>
      </div>
