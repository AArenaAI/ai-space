...
当前聊天思考块存在两个相互冲突的体验目标：

1. 推理/思考内容过长时，不应持续把最终回答顶到首屏外，导致用户迟迟看不到回答。
2. 思考块点击折叠/展开应自然、顺畅，不应出现生硬跳变或内部小滚动条造成的割裂感。
...
这可以限制思考块高度，但副作用是内容较长时会在思考块内部出现滚动条。用户反馈这类内部滚动条观感不好，尤其和折叠/展开动画叠加后，会显得不够自然。

### 当前问题

- `max-h-64 + overflow-y-auto` 能防止思考块压住回答，但会产生内部滚动条。
- 完全取消高度限制后，长推理内容可能重新占据大量可视区域。
- 生成中、完成后、分享页、历史消息等路径的思考块行为需要统一，不能只改一个组件。

### 后续目标

设计一套更自然的思考块治理方案：

- 生成中默认展示适量思考内容，避免遮住回答。
- 不使用明显的内部小滚动条作为主要交互。
- 支持点击展开/折叠，动画保持平滑。
- 完成后的思考块默认更收敛，但用户可展开查看完整推理。
- 长内容可考虑使用渐隐遮罩、摘要预览、展开按钮、主聊天区域滚动等方式，而不是内部滚动条。

### 建议实现方向

1. **生成中状态**
   - 默认展示一段有限高度的预览。
   - 底部使用渐隐遮罩和“展开思考”按钮提示，而不是直接显示内部滚动条。
   - 展开后交给聊天主滚动区域承载长内容。

2. **完成状态**
   - 默认折叠或只显示简短摘要，例如“思考推理完成”。
   - 点击后平滑展开完整 Markdown 思考内容。

3. **组件统一**
   - 优先统一以下路径：
     - `frontend/components/chat/StreamingText.tsx`
     - `frontend/components/chat/ThinkBlock.tsx`
     - `frontend/components/share/ShareContent.tsx`
     - `frontend/components/share/ShareView.tsx`
   - 避免聊天页、历史消息、分享页出现不同折叠规则。

4. **验证要求**
   - 增加浏览器 fixture 验证：
     - 点击折叠/展开时高度有中间态，不是瞬间跳变。
     - 长思考内容不出现明显内部滚动条。
     - 完成后默认收敛，展开后 Markdown 正常渲染。
   - 跑：
