...
type File struct {
	...
	VisionPromptTokens     int       `json:"vision_prompt_tokens"`     // Vision API prompt_tokens
	VisionCompletionTokens int       `json:"vision_completion_tokens"` // Vision API completion_tokens
	VisionTotalTokens      int       `json:"vision_total_tokens"`      // Vision API total_tokens
	VisionCostRMB          float64   `json:"vision_cost_rmb"`          // Vision API 实际 RMB 花费
...
type ConversationFile struct {
	ID             uint `gorm:"primaryKey" json:"id"`
	ConversationID uint `gorm:"not null;index;uniqueIndex:idx_conv_file" json:"conversation_id"`
	FileID         uint `gorm:"not null;index;uniqueIndex:idx_conv_file" json:"file_id"`
}
